...
const SignUpFormEx = () => {
   ...
   const [idInputCSS, setIdInputCSS] = useState({ color: "black" });
   ...
   const changeInput = (e) => {
      if (e.target.name === "photo") {
         ...
         if (e.target.name === "id") {
            axios
               .get(`http://localhost:7777/id.chk?id=${e.target.value}`)
               .then((res) => {
                  if (res.data.result === "중복된 아이디 입니다") {
                     setIdInputCSS({ color: "red" });
                  } else {
                     setIdInputCSS({ color: "black" });
                  }
               });
         }
